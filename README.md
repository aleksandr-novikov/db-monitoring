...
**1. Мониторируемая БД** — создаёт таблицы `users`, `products`, `orders`, `events` с ~350k строк и контролируемыми дефектами:

```bash
python -m scripts.seed_target_db
# Опционально — уменьшить объём для быстрого старта:
python -m scripts.seed_target_db --users 10000 --orders 20000 --events 50000
```
